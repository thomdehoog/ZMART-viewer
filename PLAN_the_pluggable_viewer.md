...
1. **Speak micrometres, and say which.** Name the frame rather than the unit.
   ...
   fixed in the plug contract: `CONTROLS.md` leaves it to the instrument, on
...
4. **Declare what it cannot do.** Keep `drawsUnder` / `drawsUnderBecause`.
   **Do not** make it throw — `contract.md` already decided the opposite for a
...
on the shared side. The real cost of sharing is that `RESULTS.md` must be
...
4. **A staleness check** in `run_tests.py` and `conftest.py`. The harness bundle
   ...
   measures yesterday's code. The pattern to copy already guards `frontend/dist`.
...
6. **Make the foreign-store test green** — teach `boot()` to honour
   `coverage: null` as `contract.md` already permits — and add a placement
...
10. **Freeze `viv-inside`** out of the live set — the page has already dropped
    it. Keep the folder and its `RESULTS.md` column with a dated note.
...
2. Is `RESULTS.md` re-measured, or frozen with a note?
