...
def a_stack(moment: int, *, retaken: bool = False) -> np.ndarray:
    ...
    for channel in range(COLOURS):
        for plane in range(PLANES):
            stack[channel, plane] = the_stamp(moment, channel, plane) + (50000 if retaken else 0)
    return stack
...
def a_grown_run(folder, *, timepoints=3):
    ...
    return LivePublisher(folder, profile, run_id="under-fire", cells={GridCell(0, 0): "p00"})
    ...
    try:
                ...
                for channel in range(COLOURS):
                    for plane in range(PLANES):
                        ...
                        if body is None:
                            continue
                        decoded = decode(
                            body, piece, str(composer.mosaic.dtype), composer.mosaic.axes
                        )
                        ...
                        if seen != old and seen != new:
                            torn.append(
                                f"(t={moment}, c={channel}, z={plane}) decoded {sorted(seen)[:4]}"
                            )
            return True

        with ThreadPoolExecutor(max_workers=6) as pool:
            readers = [pool.submit(hammer, wave) for wave in range(3)]
            run.replace_a_position("p00", a_stack(0, retaken=True), timepoint=0)
        ...
        for channel in range(COLOURS):
            for plane in range(PLANES):
                ...
                decoded = decode(body, piece, str(fresh.mosaic.dtype), fresh.mosaic.axes)
                assert set(np.unique(decoded[:FRAME, :FRAME])) == {the_stamp(1, channel, plane)}
    finally:
        opened.close()
...
def test_concurrent_fire_across_every_axis_stays_frame_pure(tmp_path):
    ...
    try:
        composer = opened.composer()
        addresses = [
            (0, plane, 0, 0, moment, channel)
            for moment in range(3)
            for channel in range(COLOURS)
            for plane in range(PLANES)
        ]
        quiet = {address: composer.bytes_for(*address) for address in addresses}
        with ThreadPoolExecutor(max_workers=8) as pool:
            for _ in range(3):
                answers = list(
                    pool.map(lambda address: composer.bytes_for(*address), addresses * 2)
                )
...
def test_malformed_and_boundary_addresses_answer_absent_never_crash(tmp_path):
    ...
    for inside in hostile:
        assert served.built_bytes_behind(store, inside) is None, (
...
def test_a_large_mostly_unwritten_room_costs_almost_nothing(tmp_path):
    ...
    try:
        ...
        assert spent < 2.0, (
            f"answering 116 absent frames took {spent:.2f}s -- the unwritten tail is being paid for"
        )
        ...
        assert built <= 4, f"{built} slabs were built for ground that is entirely absent"
    finally:
        opened.close()
