...
class AMicroscope:
    ...
    def image_a_position(
        self, name: str, pixels: np.ndarray, *, location_yx: tuple[float, float], timepoint: int = 0
    ) -> Path:
        ...
        if not (member / "zarr.json").exists():
            self._describe_a_member(member, pixels.shape[1], pixels.shape[2], location_yx)
        squeeze = Zstd(level=1)
        for channel in range(pixels.shape[0]):
            plane = pixels[channel].astype("<u2")
            for level in range(self.levels):
                chunk = member / str(level) / "c" / str(timepoint) / str(channel) / "0" / "0" / "0"
                chunk.parent.mkdir(parents=True, exist_ok=True)
                chunk.write_bytes(squeeze.encode(np.ascontiguousarray(plane).tobytes()))
                plane = _halved(plane)
        self._members[name] = max(self._members.get(name, 0), timepoint + 1)
        self._declare()
        return member

    def _describe_a_member(
        self, member: Path, height: int, width: int, location_yx: tuple[float, float]
    ) -> None:
        ...
        for level in range(self.levels):
            factor = 2**level
            shape = [self.timepoints, self.channels, 1, height // factor, width // factor]
            _atomically(
                member / str(level) / "zarr.json",
                json.dumps(
                    {
                        "zarr_format": 3,
                        "node_type": "array",
                        ...
                        "chunk_grid": {
                            "name": "regular",
                            "configuration": {"chunk_shape": [1, 1, 1, shape[3], shape[4]]},
                        },
            ...
            datasets.append(
                {
                    ...
                    "coordinateTransformations": [
                        {
                            "type": "scale",
                            "scale": [1.0, 1.0, 1.0, size_y * factor, size_x * factor],
                        },
                        {
                            "type": "translation",
                            "translation": [
