...
def _write(
    path: Path, data, dims, version: str, *, scale=None, scale_factors=(2,), dressed: bool = False
) -> None:
    """One store, written the ordinary way and optionally dressed."""
    import ngff_zarr

    scale = scale or {axis: _SCALE.get(axis, 1.0) for axis in dims if axis in ("z", "y", "x")}
    image = ngff_zarr.to_ngff_image(data, dims=dims, scale=scale)
    multiscales = ngff_zarr.to_multiscales(image, scale_factors=list(scale_factors))
...
def main() -> None:
    ...
    for version in ("0.4", "0.5"):
        ...
        _write(_AWKWARD / f"t_of_one_v{v}.zarr", volume[None], ("t", "c", "z", "y", "x"), version)
        # No pyramid at all: one level is a legal image.
        _write(
            _AWKWARD / f"no_pyramid_v{v}.zarr",
            volume,
            ("c", "z", "y", "x"),
            version,
            scale_factors=(),
        ...
        _write(_AWKWARD / f"one_channel_kept_axis_v{v}.zarr", lone, ("c", "z", "y", "x"), version)
        ...
        _write(
            _AWKWARD / f"uint8_v{v}.zarr",
            (volume // 256).astype("uint8"),
            ("c", "z", "y", "x"),
            version,
        )
        _write(
            _AWKWARD / f"float32_v{v}.zarr",
    ...
    _write(_AWKWARD / "undressed_v04.zarr", volume, ("c", "z", "y", "x"), "0.4")

    # A bare group wrapping one image — the wrapper itself declares nothing.
    wrapper = _AWKWARD / "wrapped_group"
    wrapper.mkdir()
    _write(wrapper / "inner.zarr", volume, ("c", "z", "y", "x"), "0.4", dressed=True)

    # Names the file system allows and code forgets: spaces and unicode.
    _write(
        _AWKWARD / "awkward name with spaces.zarr",
        volume,
        ("c", "z", "y", "x"),
        "0.4",
        dressed=True,
    )
    _write(_AWKWARD / "münster_ω.zarr", volume, ("c", "z", "y", "x"), "0.4", dressed=True)
