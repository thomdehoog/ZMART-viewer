...
def test_the_prefilled_blocks_hold_what_zarr_would_read(tmp_path):
    """Every block the warm's fast decode cached equals the zarr read of it.

    The prefill undoes a chunk's bytes directly — resolver, byte order,
    zstd — instead of going through zarr, and a decode that was subtly
    wrong would surface later as a patch laying wrong pixels with no error
    anywhere. So every cached block is held against the bytes zarr itself
    returns for the same region, trims at the edges included.
    """
    import numpy as np
    ...
    try:
        ...
        with made._block_guard:
            held = dict(made._blocks)
        assert held, "the prefill must have decoded blocks into the cache"
        for (path, outer, at), block in held.items():
            array = _zarr.open_array(str(path), mode="r")
            size = tuple(array.chunks[-3:])
            expected = np.asarray(array[outer + (
                slice(at[0] * size[0], (at[0] + 1) * size[0]),
                slice(at[1] * size[1], (at[1] + 1) * size[1]),
                slice(at[2] * size[2], (at[2] + 1) * size[2]),
            )])
            assert np.array_equal(block, expected), (
                f"block {at} of {path.name} decoded differently from what "
                "zarr reads — the fast path would hand the patcher a "
