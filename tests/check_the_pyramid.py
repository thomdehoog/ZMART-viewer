...
def read_the_built_picture(
    composer: Composer, level: int, plane: int, top: int, left: int, height: int, width: int
) -> np.ndarray:
    ...
    for row in range(top // piece, (top + height - 1) // piece + 1):
        for column in range(left // piece, (left + width - 1) // piece + 1):
            ...
            if not (0 <= row < down and 0 <= column < across and 0 <= plane < deep):
                continue
            built = composer._slab_for(level, plane, row, column)
            depth = composer.slab_depth(level)
            slab = built[plane - (plane // depth) * depth]
            from_y, to_y = max(top, row * piece), min(top + height, (row + 1) * piece)
            from_x, to_x = max(left, column * piece), min(left + width, (column + 1) * piece)
...
def how_it_lines_up(fine: np.ndarray, coarse: np.ndarray) -> tuple[int, int, float]:
    ...
    window = coarse[SHIFTS : coarse.shape[0] - SHIFTS, SHIFTS : coarse.shape[1] - SHIFTS].astype(
        "float64"
    )
...
def best_shift(fine: np.ndarray, coarse: np.ndarray) -> tuple[int, int, float]:
    ...
    for down in range(-SHIFTS, SHIFTS + 1):
        for across in range(-SHIFTS, SHIFTS + 1):
            window = coarse[
                SHIFTS + down : coarse.shape[0] - SHIFTS + down,
...
def a_tile_on_its_own(mosaic, level: int, shrink: tuple[int, int, int]) -> tuple[int, int, float]:
...
def main() -> None:
    ...
    print(f"  a patch of stage at y {middle_um[1]:.1f}, x {middle_um[2]:.1f} um\n")
    print(
        f"  {'copy':>5} {'voxel um':>9} | {'the tile alone':>22} | "
        f"{'the built picture':>22} | verdict"
    )
    print(
        f"  {'':>5} {'':>9} | {'shift':>10} {'better by':>11} | {'shift':>10} {'better by':>11} |"
    )
    print("  " + "-" * 88)
    ...
    for level in range(1, mosaic.levels):
        ...
        coarse = read_the_built_picture(composer, level, plane, top, left, PATCH, PATCH)
        ...
        print(
            f"  L{level:>4} {voxel[1]:>9.2f} | "
            f"{f'{alone[0]}, {alone[1]}':>10} {alone[2]:>10.0%} | "
            f"{f'{built[0]}, {built[1]}':>10} {built[2]:>10.0%} | "
            + ("adds nothing" if ours == 0 else f"BUILDING ADDS {ours} voxel")
        )

    beyond = (
        "no misalignment beyond what the tiles themselves have"
        if added == 0
        else f"{added} voxel of its own"
    )
