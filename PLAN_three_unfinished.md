...
* **Use `zmart-viz`.** It has zarr and Playwright. `zmart-microscopy` has
  ...
  you to use.
* `viz_studio/tests/conftest.py` reads `PLAYWRIGHT_BROWSERS_PATH` and does **not**
...
* **Rebuild the harness bundle before every viz_studio run.**
  `viz_studio/options/harness/dist/` is gitignored and `run_tests.py` builds it
  *only if it is missing* — there is no staleness check, unlike the one guarding
  `frontend/dist`. Editing either `viewer.js` or the harness and then running the
...
or `options/`; four of its tests read metadata with `json.loads`, and the fifth
drives `window.zmartViewer` — the standalone `viz_studio` frontend, a different
...
7. **Retire `viv-inside` in `viz_studio` too, or leave it?** It is out of the
   page. `EVERY_OPTION` still parametrises three, so every foreign-store run costs
   three options rather than two. If retired, keep its `RESULTS.md` column with a
...
"carries all three engines" while asserting two. `viz_studio/TESTING.md` names a
...
**The harness refuses a store with no imaged regions**, though `contract.md`
...
`CONTROLS.md` for the contrast control; that file does not mention contrast. The
real statements are in `options/contract.md` and `NEXT_STEPS.md`.**
...
row of `RESULTS.md` changes when this is fixed** — it needs a new check
