...
Written 5 August 2026, after a screening pass over `viz_studio/` and `zmart_storage/`
...
| coverage of `viz_studio/backend` | `server.py` 87%, `stores.py` 89%, `announcements.py` 99%, `linking.py` 86% |
...
|---|---|---|---|---|
| `viz_studio/backend/server.py` | 1521 | **629** | 381 | 374 |
| `viz_studio/backend/stores.py` | 1182 | **317** | 212 | 470 |
...
collection of every one of its 622 tests, so `python viz_studio/run_tests.py` — the
...
decimation is at line 1675 now. `ARCHITECTURE.md` already records that citing line
...
| what | lines | risk |
|---|---|---|
| Delete the legacy pointer-map locations in `viz_studio/backend/linking.py` — two of the three places it looks are unreachable, so the compatibility they claim does not exist | −60 | very low |
...
`HANDOVER_a_view_that_writes_nothing.md` invites somebody to decide whether it is
...
**`viz_studio/options/` (13 617 lines) is an unfinished comparison, not an abandoned
...
| Drop the two Viv options from `options/` | −5 061 | the three-way comparison that is waiting on a measurement |
| Retire the three contact-sheet measurement scripts | −648 | the ability to reproduce `TILES_IN_ONE_STORE.md`. Marking them historical costs six lines and loses nothing |
...
| Drop `backend/resolution_demo.py` | −240 | a diagnostic volume whose bars make the pyramid level being drawn legible from the picture. Undocumented, so probably forgotten rather than retired |
