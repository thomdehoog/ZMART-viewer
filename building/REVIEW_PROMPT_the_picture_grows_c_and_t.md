...
All on branch `claude/thy1-linked-spiral`, in `viz_studio/`:
...
Chase claims into the code: `building/declare.py` (the multi-channel
refusal and what the description declares), `building/governed.py` (the
per-commit patch the bake rules extend), `building/composer.py` (what a
compose actually reads), `zmart_live/coordinator.py` and `profiles.py`
(what the writer commits and refuses), `backend/server.py` (the frames
