...
- **Windows rule, learned as WinError 5 mid-demo:** the server must not
  ...
  test.
- The 2026-08-12 decision note (`NOTE_live_updates_seen_on_screen.md`,
...
- The serving door: `viz_studio/building/served.py` (`the_bytes_behind`,
  `_the_serving_behind`); the live snapshot: `viz_studio/building/governed.py`
  (`composer`, `_compose_the_snapshot`, `_what_changed_dirtied`); the
  transfer bake: `viz_studio/building/declare.py`
  (`_bake_the_coarse_ground`); refresh contracts:
  `viz_studio/tests/test_the_composer_obeys_the_manifest.py`,
  `viz_studio/tests/test_frontend_live_refresh_contract.py`.
- Suites: `python -m pytest viz_studio/tests zmart_live/tests zmart_storage`
...
- The scale harness: `viz_studio/building/measure_a_governed_run_at_scale.py`
