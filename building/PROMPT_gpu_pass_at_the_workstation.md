...
structure follows `viz_studio/building/CONTRACT_the_files_the_viewer_needs.md`,
...
The full background lives in `viz_studio/building/PLAN_close_the_neuroglancer_chapter.md`
...
1. **Clean build.** `cd viz_studio/frontend && npm install && npm run build`.
...
2. **The browser gates, on the real GPU.** From `viz_studio/`:
...
5. **Review finding C4 — the one open review item.** Twenty minutes reading
   ...
   (`viz_studio/frontend/node_modules/neuroglancer/lib/chunk_manager/backend.js`,
