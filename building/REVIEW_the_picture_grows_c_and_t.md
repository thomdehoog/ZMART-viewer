...
(`viz_studio/building/governed.py:584-586` calls `_keep_the_bake_true` before
...
replacement costs 330–380 ms today (`PLAN_close_the_neuroglancer_chapter.md:33`);
...
(`PLAN_the_picture_grows_a_z_axis.md:154-159`). The c-and-t plan, whose axis
...
- Dirty footprints are `(level, row, column)` in every consumer — the derive
  ...
  sabotage test (`PLAN_the_picture_grows_a_z_axis.md:141-147`).
...
(`PLAN_close_the_neuroglancer_chapter.md:220-221`); nothing of it exists. In
...
(`PLAN_close_the_neuroglancer_chapter.md:37`). A torn stamp, a rollback, or a
...
- The backend's governed-file door hard-codes the same shape: "A chunk file
  ...
  the store is the fifth parent" (`backend/server.py:456-459`, code at
  466-472), and `_built` peels `parts[-4] != "c"` (`backend/server.py:536`).
...
(`PLAN_the_picture_grows_a_z_axis.md:89-90`). This plan's preamble binds it to
...
- *Warm equals reload, all zoom bands, both channels, two moments:* the F5
  ...
  (`PLAN_the_picture_grows_a_z_axis.md:170-173`) — but this plan builds no
...
(`TESTPLAN_the_picture_grows_a_z_axis.md:135-137`); under the decided
...
(`TESTPLAN_the_picture_grows_a_z_axis.md:114-125`): tiny governed fixture
...
2. **"The collection declaration names each member's written moments."** The
   ...
   (`viz_studio/tests/a_microscope.py:176`) and in the contract as a promise
   ("which rides beside the list *when time lands*",
   `CONTRACT_the_files_the_viewer_needs.md:296-298`). The arrival signal the
...
4. **The writer's refusal is not a remedy.** The message at
   ...
   (`PLAN_the_picture_grows_a_z_axis.md:122-125`). One sentence of work, but
...
(`PLAN_the_picture_grows_a_z_axis.md:117-129`). Deciding the same way here is
...
- "Un-imaged room must cost nothing per landing." Today's walkers do not
  ...
  (`PLAN_the_picture_grows_a_z_axis.md:143-147`,
...
(`PLAN_close_the_neuroglancer_chapter.md:79-95`), so item 3's server-side
...
- *Shown by construction:* a page holding a **non-current** moment (the
  ...
  (`PLAN_the_picture_grows_a_z_axis.md:42-51`), smaller only because a
...
- *Suspected, with the bench on its side:* the first visit to an old moment
  ...
  bytes at the server (`TESTPLAN_the_picture_grows_a_z_axis.md:48-55`).
...
(`frontend/src/AxisSlider.jsx:104-131`), capped by the server's
frames count (`backend/server.py:1686-1688`, `stores.py:768` ff). And the
