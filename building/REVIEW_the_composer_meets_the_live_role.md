...
In the real serving path, `viz_studio/backend/server.py:352` gates on the
...
through to `_built` (`backend/server.py:397, 446-472`), whose composer then
reads the run's position bytes **directly through zarr**, invisible to the
gate. `backend/server.py:458-461` even documents that the tiles "are **not**
...
- Nothing catches exceptions from `served.the_bytes_behind`:
  `backend/server.py:397` calls it bare, and `composer.bytes_for` →
  ...
  (`building/server.py:112-119`) states the consequence: the engine "treats
...
- **`isdigit`/`int` mismatch crashes instead of 404**: `served.py:123-125`
  and `building/server.py:89-95` validate with `str.isdigit()` then call
  ...
  try/except.
- **No cache headers on the standalone server**: `building/server.py:_send`
  ...
  (`backend/server.py:485-486`), but only when `self._live` is set — a flag
...
- **Dead rotation support with a subtle trap**: `Tile.footprint`
  ...
  at the door.
- **Ledger races in the measurement server**: `building/server.py:69-70, 76,
