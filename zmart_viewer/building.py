...
class TheWorldFrame(Mosaic):
    ...
    def frame_room(self) -> tuple[int, int]:
        ...
        return (
            max(int(self._profile.timepoints), from_tiles[0]),
...
class ComposedPicture:
    """A changing composed view with shared, chunk-wise coarse-file updates."""

    def __init__(self, store: Path | None, piece: int):
...
class GovernedRun(ComposedPicture):
    ...
    def _compose_the_snapshot(
        ...
        return (
            Composer(
                TheWorldFrame(ordered, layout, profile, run=self.folder),
                piece=self._piece,
            ),
            drawing,
            tiles,
