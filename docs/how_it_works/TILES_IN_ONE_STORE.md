...
stage really went. `docs/how_it_works/DATA_LAYOUT.md` Decision 1b records the cost of writing tiles
...
because the fused image no longer holds the overlap. `docs/how_it_works/DATA_LAYOUT.md` Decision 1b
...
`docs/how_it_works/DATA_LAYOUT.md`, where tiles straddling a piece boundary lost up to 75% of their
...
- **No two tiles share a piece of the file**, because no two tiles share ground.
  The hazard measured in `docs/how_it_works/DATA_LAYOUT.md` — two tiles straddling a piece boundary,
...
`docs/how_it_works/INTEROP.md` §3 describes the cure — a cosine ramp from each store's declared
...
| `docs/how_it_works/DATA_LAYOUT.md` Decision 1b | why one image normally destroys the overlap |
| `docs/how_it_works/INTEROP.md` §3 | the cosine ramp that would soften the seams |
