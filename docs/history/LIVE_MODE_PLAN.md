...
`docs/how_it_works/DATA_LAYOUT.md` Decision 2, which describes a timelapse growing its own array, is the
...
`docs/how_it_works/DATA_LAYOUT.md` records the viewer opening on an empty grey rectangle for weeks with
...
of this plan offered as fixture models use the flat layout, which `docs/how_it_works/DATA_LAYOUT.md`
forbids.
...
Measured in `docs/open/NEXT_STEPS.md`, a tile landing costs 22 requests, paid on every announcement.
...
`docs/how_it_works/DATA_LAYOUT.md` offers the stage's travel limits — **but that default was justified for an
...
- `docs/how_it_works/DATA_LAYOUT.md` Decision 2 matches what `stores.py` already does, and the separator
