...
> `docs/how_it_works/DATA_LAYOUT.md` — a viewer that cannot reach an instrument can be opened on
...
`docs/history/INTEGRATION_ROADMAP.md`: the roadmap says *why* and *in what order at the
...
**The engine half is done** (`app/page/src/`, `backend/`):
...
- `app/server/server.py` already serves the image as OME-Zarr under `/data`,
...
Everything lands in the spike's stack (`app/page`, `app/server`);
...
- **`app/page/src/App.jsx`** — extend `shaderFor()`/`layersFor()` so the live
...
- **New `app/page/src/annotations.js` (or a component)** — create the neuroglancer
...
- **`app/server/server.py`** — add `GET/POST /api/annotations` to persist what was
  drawn beside the store, and have `/api/config` include a coarse per-channel
  histogram and (when present) the time axis length.
- **`app/server/stores.py`** — report the store's dimensions including `t`, so the
