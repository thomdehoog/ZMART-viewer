...
>
> **Read `docs/open/HANDOVER_overlapping_runs.md` instead.** It describes what was built and
...
`docs/how_it_works/DATA_LAYOUT.md` Decision 1b measures the loss at **21% of everything the camera
...
But the viewer wants one image, for reasons `docs/open/NEXT_STEPS.md` establishes at length:
...
`docs/how_it_works/DATA_LAYOUT.md` took it on a **9 × 9 raster — 81 tiles — overlapping by 12%, four
...
near the cliff, which `docs/open/NEXT_STEPS.md` puts between a hundred positions at 302
...
It was set aside for one reason, recorded at `docs/how_it_works/DATA_LAYOUT.md`: it "leaves every
...
Measured in `docs/how_it_works/TILES_IN_ONE_STORE.md`: about 6 ms a piece, exact to the voxel, and
...
`docs/how_it_works/INTEROP.md` is this project's own record of what happens when files and readers
...
problem this document exists to remove. `docs/how_it_works/TILES_IN_ONE_STORE.md` says plainly that
...
`docs/how_it_works/INTEROP.md` §3 offers a cosine ramp from each image's declared edge as the cure,
"about fifteen lines". That estimate was made against a Viv prototype and **does
not carry to the engine we actually ship on.** `app/page/src/scene.js` records why,
...
- **Where does it live?** A pyramid belongs to one image, so a combined one is a
  ...
  engine's own job and the thing `docs/open/NEXT_STEPS.md` §1 declines to take over.
- **Who writes it during a run?** `docs/how_it_works/ARCHITECTURE.md` §7 already calls keeping these
...
- **It brings back the hazard four images removed.** A combined image is one image
  ...
  fault `docs/how_it_works/DATA_LAYOUT.md` measures at up to three quarters of a tile lost, silently.
- **It is not as cheap as "a tenth of the data" suggests.** `docs/how_it_works/DATA_LAYOUT.md` records
...
**A dataset must be able to be four stores.** `docs/how_it_works/ARCHITECTURE.md` §3 already says a
...
aligning is worth doing because `docs/how_it_works/DATA_LAYOUT.md` already requires it of the writer
...
2. **The overlap survives.** Every voxel the camera recorded is readable afterwards.
   `docs/how_it_works/DATA_LAYOUT.md`'s existing measurement of 0.0% overwritten is the model.
...
— 2D, 3D, channels, timepoints — are written into `docs/how_it_works/TILES_IN_ONE_STORE.md` with the
...
- **"Synthesising the served chunk keys frees the separator decision."** It does
  not. Both mechanisms in `docs/history/LIVE_MODE_PLAN.md` §2 read the disk, never the served
...
- **"Measuring it in the viewer is the point of the exercise."** Largely
  pre-answered: `docs/history/WHERE_THINGS_STAND.md` records no interface freeze even with the
