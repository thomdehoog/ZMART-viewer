...
> written to disk has been superseded by `docs/how_it_works/DATA_LAYOUT.md`, which is the document to
> trust. If you want to know how the viewer works today, read `parked/prototype/README.md` and
> `docs/how_it_works/DATA_LAYOUT.md` instead.
...
- `zmart-viewer/` runnable app with a one-command **demo** launch.
...
3. **Where it lives** → **In-repo `zmart-viewer/`** for the spike. One clone, one
...
`app/server/demo_data.py` synthesizes the "at the microscope" volume: a
