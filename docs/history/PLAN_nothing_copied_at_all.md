...
`docs/how_it_works/LINKING_INSTEAD_OF_COPYING.md` said, and `zmart_storage/linked.py` repeated, that
...
`docs/history/PLAN_showing_many_stores_as_one.md` sets out, applied to a coarser grid, and it was
...
- **The full-resolution alignment rule still applies**, and a drifted run is still
  ...
  run open. That is `docs/history/PLAN_showing_many_stores_as_one.md`.
