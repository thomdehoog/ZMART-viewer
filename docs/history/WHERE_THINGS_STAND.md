...
**Three stale documents** (`fc0f64b`). `docs/how_it_works/ARCHITECTURE.md` §3 described departures
that have all been closed. `docs/how_it_works/DATA_LAYOUT.md` described rows gathered by acquisition
...
`docs/how_it_works/DRAWING_ENGINES.md`.
...
and nothing reports that it has. Recorded in `docs/how_it_works/CONTROLS.md`.
...
the operator presses save, before the run starts. Recorded in `docs/how_it_works/LAYERS.md` and
`docs/history/OPTIONS.md`.
...
**Telling "imaged and dark" from "never visited"** — `docs/history/OPTIONS.md`. Writing a floor
...
**Drawing nothing until there is something to draw** — `docs/history/OPTIONS.md`. Instead of one
...
- I cited `docs/how_it_works/DATA_LAYOUT.md` for a rule that is actually in `zmart_storage/canvas.py`.
...
- A layer given a source address beginning with `/` is never fetched and **nothing
  ...
  builds on this next.
- `zmart-viewer/INDEX.md` still describes the design as "one store per position" and
...
Their results will be in `zmart-viewer/parked/RESULTS.md` and `zmart-viewer/LAYER_STACK.md`.
