...
*Measured 2026-07-30 (`docs/how_it_works/DRAWING_ENGINES.md`, and the live-tiles work).* Registration
 ...
 *                 description of its colours — see parked/contract.md §6
...
1. **Registration** — `zmart-viewer/tests/margins.py`. A square of image with a
   ...
   **That number alone is not enough, and knowing why is half the measurement.**
   ...
   in return. Both are reported. `parked/RESULTS.md` row 1c shows the second one catching
...
2. **Handedness** — open an acquisition written dim at one edge and bright at the
   ...
   shipped for months because nothing asked this; see `docs/how_it_works/CONTROLS.md` §1a and
...
3. **Two gestures and no more** — drag pans, the wheel zooms, and every other
   gesture leaves the picture byte-identical. See `docs/how_it_works/CONTROLS.md`.
...
5. **New data arriving while somebody is watching** — the measurement that matters
   ...
   - **Does it appear at all**, and what call was needed to make it — nothing on
     ...
     path `app/page/src/engine.js` already uses, and Viv by being handed a fresh
