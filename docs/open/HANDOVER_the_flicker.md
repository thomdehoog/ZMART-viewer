...
(`app/server/make_awkward_stores.py` + `app/server/sweep_awkward_stores.py`)
...
the frontend at every step (`npm --prefix app/page run build`), because the
...
`app/page/src/engine.js` answered a revision advance in two steps: refresh
