...
| | |
| --- | --- |
| `docs/open/HANDOVER_overlapping_runs.md` | this file — what was built, and the numbers |
| `docs/how_it_works/LINKING_INSTEAD_OF_COPYING.md` | showing a run without copying it — now built, and what it still needs |
| `docs/history/PLAN_showing_many_stores_as_one.md` | the next piece of work, written out step by step: showing a folder of stores as one picture without rewriting any of them, so that a real transfer opens |
| `docs/how_it_works/ARCHITECTURE.md` §7 | the three layers the whole tool is made of |
| `docs/open/OPEN_a_run_that_changes_while_you_watch.md` | an open question — a new position works, but gaining a colour or a moment, or re-imaging a position, does not |
| `docs/history/PLAN_keeping_the_overlap.md` | **superseded** — two arrangements that were weighed and not built |
| `docs/how_it_works/TILES_IN_ONE_STORE.md` | the bench measurements behind all of it |
...
`docs/open/HANDOVER_3D.md` has the three commands for getting the volume view up.
...
`docs/how_it_works/LINKING_INSTEAD_OF_COPYING.md` predicts, turning up in a measurement.
...
`docs/how_it_works/ARCHITECTURE.md` §7 as unsolved.

---

`docs/how_it_works/INTEROP.md` §5 explains why live stitching is slow, and it is not what you would
...
- **Dealing tiles across four images** so neighbours never share one. Already
  measured in `docs/how_it_works/DATA_LAYOUT.md` at nothing lost and no loss of drawing rate — but
...
- **Keeping tiles in slots and placing them as they are read.** Measured and quick,
  and it makes the store meaningless to any software but ours. `docs/history/PLAN_keeping_the_overlap.md`
...
- **Rotation in the volume does not work**, and its brightness is computed with the
  ...
  described in `docs/open/HANDOVER_3D.md`, and both are the smallest clearly specified work
  available.
...
**A measurement flattered itself.** The by-level table in `docs/how_it_works/TILES_IN_ONE_STORE.md`
...
four-image arrangement was measured and written down in `docs/how_it_works/DATA_LAYOUT.md` long ago,
