...
| S | A multi-colour acquisition showed only its first channel, in white. The three options in `parked/` (in the ZMART-microscopy repository) made the *page* responsible for describing a run's channels and gave it no way to learn them, so pages said nothing and every option fell back to one white channel | measured: on a run recorded in two colours, 4.8% of the window green and 4.7% red after the fix, against one white half and nothing else before it. Fixed 2026-08-01; see `parked/contract.md` §6 |
