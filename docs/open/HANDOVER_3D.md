...
> `emptyTheEnginesOwnBindings` (in `parked/neuroglancer-under/viewer.js`) does not
...
at all. This one is about `zmart-viewer`'s own viewer rather than the
`parked/neuroglancer-under` one above, but the trap is the same in both and it
...
2. **The sweep varied the wrong zoom.** `crossSectionScale` and `projectionScale`
   ...
   changed" was duly reported. `app/page/src/ScaleBar.jsx:90` documents this exact
...
Confirmed again afterwards on the demo volume in `zmart-viewer`, from a different
...
binding; see `docs/how_it_works/CONTROLS.md` §1.
...
| | |
| --- | --- |
| the engines | `zmart-viewer/parked/<name>/viewer.js` |
| the interface | `zmart-viewer/parked/contract.md` — `canShowVolume`, `showVolume`, `theDepthItCanShow` |
...
| what a real store said | `zmart-viewer/parked/RESULTS.md`, top section |
