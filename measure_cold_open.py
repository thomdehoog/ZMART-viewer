...
This is the script behind the cold-open figures in `NEXT_STEPS.md`. It answers one
...
The stores are shaped the way `DATA_LAYOUT.md` asks for at their coarsest level,
...
# coarsest level DATA_LAYOUT.md asks for, and the coarsest level is what the
    ...
    try:
        ...
        if result["storesRead"] > result["rows"]:
            print(
                ...
                "described in NEXT_STEPS.md has been lost. Look at build_config in "
                "backend/server.py: the measurement belongs inside the branch that "
