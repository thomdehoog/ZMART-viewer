...
`docs/open/NEXT_STEPS.md` proposes two ways to stop a large finished folder being slow to
...
It needs the viewer page built (``npm --prefix app/page run build``) and a
...
def main() -> int:
    ...
    print(
        "If first pixel grows roughly in step with the number of positions, the\n"
        "concern in docs/open/NEXT_STEPS.md is real. If it flattens, it is not."
    )
    return 0
