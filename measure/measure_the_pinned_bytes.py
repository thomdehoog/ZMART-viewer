...
def levels_of(
    height: int, width: int, depth: int, *, z_halves: bool, hold_z_levels: int = 0
...
def pinned_bytes(shapes: list[tuple[int, int, int]]) -> tuple[int, list[int]]:
    ...
    for level, (deep, tall, wide) in enumerate(shapes):
        if deep * tall * wide <= PINNED_SHARE * full:
            pinned.add(level)
    total = (
        sum(shapes[level][0] * shapes[level][1] * shapes[level][2] for level in pinned)
        * BYTES_PER_VOXEL
    )
...
def report(name: str, height: int, width: int, depth: int) -> None:
    ...
    for label, z_halves, hold in (
        ("z never halves", False, 0),
        ("z always halves", True, 0),
        ("z held 3 then halves (Thy1 shape)", True, 3),
    ):
        shapes = levels_of(height, width, depth, z_halves=z_halves, hold_z_levels=hold)
        total, pinned = pinned_bytes(shapes)
        verdict = (
            "OK"
            if total < ram * 0.25
            else ("TIGHT" if total < ram * 0.6 else "OUT OF MEMORY IN PRACTICE")
        )
        print(
            f"  {label:36s} {len(shapes)} levels, pins {pinned}, "
...
def main() -> int:
    ...
    report("40 x 40 flat survey (frame 1152, ~10% overlap)", 40 * 1040 + 112, 40 * 1040 + 112, 1)
    # The motivating evening: 49 Thy1 blocks.
    report(
        "49 Thy1 blocks (7 x 7 of 1264 x 1480 x 291, ~10% overlap)",
        7 * 1138 + 126,
        7 * 1332 + 148,
        291,
    )
    # The reviews' arithmetic case: 4,096 Thy1-shaped positions.
    report("4,096 Thy1-shaped positions (64 x 64)", 64 * 1138 + 126, 64 * 1332 + 148, 291)
    return 0
