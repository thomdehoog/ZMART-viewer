...
# The repository root as well, so zmart_viewer.record imports on a machine where the
...
def the_run(across: int, seed_value: int = 7) -> tuple[LivePublisher, list[str]]:
    ...
    run = LivePublisher(
        folder,
        profile,
        run_id=f"scale{across}",
        cells=cells,
        timepoints=1,
        linked_view="at_run_end",
    )
    order = [
        f"p{row:0{width}d}{column:0{width}d}" for row in range(across) for column in range(across)
    ]
    ...
    if not (wanted / "zarr.json").is_file():
        ...
        for number, position_id in enumerate(order):
            run.write_a_position(
                position_id, seed.integers(*BRIGHT, (1, FRAME, FRAME)).astype("uint16")
            )
            if (number + 1) % 200 == 0:
                print(f"  {number + 1} written ({(time.time() - began):.0f} s)", flush=True)
...
def main() -> int:
    ...
    parsing.add_argument(
        "--across", type=int, default=32, help="the survey is this many positions on a side"
    )
    parsing.add_argument(
        "--churn",
        type=int,
        default=40,
        help="how many watched changes: half boundary landings, half interior replacements",
    )
    parsing.add_argument(
        "--burst",
        type=int,
    ...
    parsing.add_argument("--headed", action="store_true", help="open a visible window")
    parsing.add_argument(
        "--fixtures",
        default=None,
    ...
    if to_publish:
        print(
            f"Bulk-publishing {len(to_publish)} interior positions "
        ...
        for number, position_id in enumerate(to_publish):
            ...
            if (number + 1) % 500 == 0:
                print(f"  {number + 1} published ({(time.time() - began):.0f} s)", flush=True)
    ...
    if asked.as_declared:
        ...
        if asked.bake:
            print("Declaring with the per-commit bake (initial bake is O(survey), once)...")
        began_declaring = time.time()
        store = declare_a_governed_picture(shown, run.folder, name="live", bake=asked.bake)
        print(
            f"declared {'baked' if asked.bake else 'unbaked'} in "
    ...
    def one_frame(told) -> None:
        frames.append((float(told["metadata"]["timestamp"]), base64.b64decode(told["data"])))
        session.send("Page.screencastFrameAck", {"sessionId": told["sessionId"]})
    ...
    try:
        ...
        try:
            with urllib.request.urlopen(
                f"http://127.0.0.1:{port}/data/0/{store.name}/3/c/0/0/0", timeout=120
            ) as answer:
                answer.read()
        ...
        if asked.warm_first and governed is not None:
            warming = time.time()
            while time.time() - warming < 300 and not governed.composer().coarse_levels_are_warm:
                page.wait_for_timeout(500)
            print(
                f"coarse slabs warm {time.time() - warming:.1f} s after "
        ...
        derives_before = 0
        if asked.burst:
            dark = [one for one in boundary if (one, 0) not in set(run._committed_units())]
            landing = dark[: asked.burst]
            derives_before = int(dict(getattr(governed, "accounting", {})).get("derives", 0))
            print(
            ...
            while True:
                shot = np.asarray(
                    Image.open(io.BytesIO(page.screenshot())).convert("L"), dtype=np.int16
                )
                if int((shot[:, :640] > 100).sum()) > 2000:
                    print(
                        f"  ground on screen {time.time() - settling:.1f} s "
                ...
                if time.time() - settling > 180:
                    print(
                        "  no ground on screen after 180 s -- bursting "
                        "against a dark canvas, which the screenshots "
                        "will say"
                    )
                    break
                page.wait_for_timeout(1000)
            page.screenshot(path=str(FIXTURES / f"gov{across}x{across}" / "burst_before.png"))
            first = time.time()
            ...
            settled_before = np.asarray(
                _Image.open(FIXTURES / f"gov{across}x{across}" / "burst_before.png").convert("L"),
                dtype=np.int16,
            )
            absorbing = time.time()
            while time.time() - absorbing < 60:
                ...
                if not later:
                    continue
                shot = np.asarray(_Image.open(io.BytesIO(later[-1])).convert("L"), dtype=np.int16)
                if (
                    shot.shape == settled_before.shape
                    and int((np.abs(shot - settled_before) > 20).sum()) >= 50
                ):
                    while (
                        frames
                        ...
                        page.wait_for_timeout(250)
                    break
            page.screenshot(path=str(FIXTURES / f"gov{across}x{across}" / "burst_after.png"))
            session.send("Page.stopScreencast")
        else:
            chooser = np.random.default_rng(11)
            seed = np.random.default_rng(23)
            half = asked.churn // 2
            landings = [one for one in boundary if (one, 0) not in set(run._committed_units())][
                :half
            ]
            victims = list(chooser.choice(sorted(interior), size=half, replace=False))
            ...
            print(f"  {'change':>22} {'writer':>8} {'derive':>8} {'read':>6} {'visible':>9}")
            for kind, position_id in plan:
                ...
                if kind == "land":
                    ...
                    brighter = seed.integers(*BRIGHT, (1, FRAME, FRAME)).astype("uint16")
                ...
                print(
                    f"  {kind:>7} {position_id:>13} {writer_ms:>6.0f}ms "
                    f"{rows[-1]['derive_ms']:>6.0f}ms {rows[-1]['read']:>6} "
                    f"{visible_ms:>7.0f}ms",
                    flush=True,
                )
            ...
            trailing = time.time()
            while time.time() - trailing < 30 and frames and time.time() - frames[-1][0] < 1.0:
                page.wait_for_timeout(250)
            session.send("Page.stopScreencast")
        ...
        grey = [
            (when, np.asarray(Image.open(io.BytesIO(body)).convert("L"), dtype=np.int16))
        ...
        for index in range(1, len(grey) - 1):
            ...
            if int(dipped.sum()) >= ENOUGH:
                ...
                print(
                    f"  TRANSIENT at frame {index}: {int(dipped.sum())} px, "
        ...
        if asked.burst:
            ...
            changing = [
                index
                for index in range(1, len(grey))
                if grey[index][1].shape == grey[index - 1][1].shape
                and int((np.abs(grey[index][1] - grey[index - 1][1]) > 20).sum()) >= 50
            ]
            if burst_landed and burst_took > 0:
                print(
                    f"  committed: {burst_landed:.0f} landings in "
                    f"{burst_took:.2f} s — "
            ...
            visibles = [one["visible_ms"] for one in rows if one["visible_ms"] == one["visible_ms"]]
            writers = [one["writer_ms"] for one in rows]
            print(
                f"  derive per commit: middling {middle(derives):.0f} ms "
            ...
            print(
                f"  writer per change: middling {middle(writers):.0f} ms "
